...
-  확률과 변수의 개념이 합쳐진 것, 측정 값이 변할 수 있는 확률이 주어진 변수
- 표본공간의 크기에 따라 나올 수 있는 확률이 달라짐

- 확률변수의 종류:
    
        이산확률변수: 변수가 가질 수 있는 값이 실숫값
        분포를 그려보면 각 값들이 점으로 찍힘

        연속확률변수: 연속형 값을 가지므로 값들의 개수를 셀 수 없음 / 특정한 값이 아닌 특정 구간의 확률로 접근

![.](image/isan.jpg) 

**심슨의 역설**

- 데이터의 세부 비중에 따라 전체 대표 확률이 왜곡되는 경우가 생기는 역설

- 확률은 데이터의 정보를 압축해서 보여주기 때문에 그 특성을 고려하지 않아 잘못된 결과가 나올 수 있음

- 데이터를 어떻게 나누고 결합하고 가공하는가에 따라 결과가 정반대로 바뀔 수 있음을 경계해야 함

- 통계적 결론이 나왔을 때 항상 의심하고 데이터의 원천 단계부터 결과의 원인을 확인해야함
